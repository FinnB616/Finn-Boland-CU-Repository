...
Q_mod2 = zeros(Count, 1); % Q term for Model #2
P_mod2 = zeros(Count, 1); % P term for Model #2
...
% /////////////////////////////////////////////////////////////////////////
    ...
    Q_mod2(n) = 1/(WingLiftModel.e(n)*(1 - 2*((Design_Input.Dia_f(n))^2/(Design_Input.Sref_w(n)*Design_Input.AR_w(n)))));
    P_mod2(n) = 0.38*Parasite_Data.CDo(n); 
    eo_mod2(n) = 1/(Q_mod2(n) + P_mod2(n)*pi*Design_Input.AR_w(n)); %Oswalds Estimate
    ...
    Model3_Name = 'Brandt'; %Name of third Oswald's Model
    eo_mod3(n) = 4.61*(1-0.045*(Design_Input.AR_w(n))^0.68)*(cosd(Design_Input.Sweep_w(n)))^0.15-3.1;
...
InducedDrag_Data = table(eo_mod1, eo_mod2, eo_mod3, k1_mod1, k1_mod2, k1_mod3, k2_mod1, k2_mod2, k2_mod3);
 ...
 %Isolated Induced Drag Coefficients
    CDi_w = (WingLiftCurve{n,:}').^2/...
        (pi*WingLiftModel.e(n)*Design_Input.AR_w(n));
    CDi_mod1 = (WingLiftCurve{n,:}').^2.*InducedDrag_Data.k1_mod1(n)...
        +InducedDrag_Data.k2_mod1(n).*((WingLiftCurve{n,:}'));
    CDi_mod2 = (WingLiftCurve{n,:}').^2.*InducedDrag_Data.k1_mod2(n)...
        +InducedDrag_Data.k2_mod2(n).*((WingLiftCurve{n,:}'));
    CDi_mod3 = (WingLiftCurve{n,:}').^2.*InducedDrag_Data.k1_mod3(n)...
...
if Plot_Induced_Data == 1
    ...
    for n=1:Count
        ...
        if Count == 1
            print('Induced Drag Plot','-dpng','-r300')
        end
    end
...
% TO RUN 1 AIRCRAFT CONFIGURATION, CHANGE COUNT VARIABLE n TO 1 (LINES 76-83, 90-93). DELETE end LINE 103 AND ADD IT LINE 70.
