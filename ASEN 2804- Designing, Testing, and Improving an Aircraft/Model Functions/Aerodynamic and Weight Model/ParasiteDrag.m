...
for n = 1:Count
    ...
    Cf_f = 0.455 / ((log10(Re_f) ^2.58) * ((1 + (0.144 * ((Design_Input.V_o(n) / ATMOS.a(n))^2))) ^ 1.65)); %Fuselage coefficient of friction
    FF_f(n) = 0.9 + (5 / ((Design_Input.Length_f(n) / Design_Input.Dia_f(n)) ^ 1.5)) + ((Design_Input.Length_f(n) / Design_Input.Dia_f(n)) / 400); %Fuselage Form Factor
    ...
    c = 0.23; %cord length
    Re_w = Design_Input.V_o(n) * c / ATMOS.nu(n); %Wing Re
    Cf_w = 0.074 / (Re_w ^ 0.2); %Wing Flat Plate Coef of Friction for Turbulent Flow
    FF_w(n) = 1 + (0.6 * 0.087 / 0.302) + (100 * (0.087 ^ 4)); %Wing Form Factor
    ...
    if Design_Input.Swet_h1(n)~=0 % If this component exists:
        Re_h1 = Design_Input.V_o(n) * 0.13 / ATMOS.nu(n); %Horz Tail Re
        Cf_h1 = 0.074 / (Re_h1 ^ 0.2); %Flat Plate Coef of Friction for Turbulent Flow
        FF_h1(n) = 1 + (0.6 * 0.1 / 0.25) + (100 * (0.01 ^ 4)); %Horz Tail Form Factor
    ...
    if Design_Input.Swet_h2(n)~=0 % If this component exists:
        Re_h2 = Design_Input.V_o(n) * 0.13 / ATMOS.nu(n); %Horz Tail Re
        Cf_h2 = 0.074 / (Re_h2 ^ 0.2); %Flat Plate Coef of Friction for Turbulent Flow
        FF_h2(n) = 1 + (0.6 * 0.1 / 0.25) + (100 * (0.01 ^ 4)); %Horz Tail Form Factor
    ...
    if Design_Input.Swet_v1(n)~=0 % If this component exists:
        Re_v1 = Design_Input.V_o(n) * 0.025 / ATMOS.nu(n); %Horz Tail Re
        Cf_v1 = 0.074 / (Re_v1 ^ 0.2); %Flat Plate Coef of Friction for Turbulent Flow
        FF_v1(n) = 1 + (0.6 * 0.01 / 0.5) + (100 * (0.01 ^ 4)); %Horz Tail Form Factor
    ...
    if Design_Input.Swet_v2(n)~=0 % If this component exists:
        Re_v2 = 0; %Horz Tail Re
        Cf_v2 = 0; %Flat Plate Coef of Friction for Turbulent Flow
        FF_v2(n) = 0; %Horz Tail Form Factor
    ...
    if Design_Input.Abase_f(n)~=0 % If this component exists:
        D_q_base(n) = (0.139 + 0.419 * ((Design_Input.V_o(n) / ATMOS.a(n)) - 0.161) ^ 2) * Design_Input.Abase_f(n);
        CDo_misc(n) = D_q_base(n)/Design_Input.Sref_w(n); %Contribution of base drag to CDo
    end

    %% Leakage and Proturbance Contribution to CDo
    L_P_Percent = 0.03; %Percent increase of total parasite drag as estimate of leakage and perturbance in fractional value (10% = 0.1)
...
Q_Table = table(Design_Input.Q_f,Design_Input.Q_w,Design_Input.Q_h1,Design_Input.Q_h2,Design_Input.Q_v1,Design_Input.Q_v2); %Compiles table of all component interference factors
