...
% /////////////////////////////////////////////////////////////////////////
    ...
    f_taper = 0.0524*Design_Input.Taper_w(n)^4 - 0.15*Design_Input.Taper_w(n)^3 + 0.1659*Design_Input.Taper_w(n)^2 - 0.0706*Design_Input.Taper_w(n) + 0.0119; % Taper ratio factor
    e_ns = 1/(1 + f_taper*Design_Input.AR_w(n));

    if Design_Input.QuarterSweep_w(n) == 0 % If no quarter chord sweep:
        e(n) = e_ns;
    else % If there is quarter chord sweep:
        e(n) = e_ns*cosd(Design_Input.QuarterSweep_w(n));
    end
    ...
    a(n)= a_0(n)/(1 + (57.3 * a_0(n)/(pi*e(n)*Design_Input.AR_w(n)))); % 3D airfoil lift curve slope
...
if Plot_Wing_Data == 1
    ...
    for n=1:Count
        ...
        if Count == 1
            print('Lift Curve Plot','-dpng','-r300')
        end
    end
